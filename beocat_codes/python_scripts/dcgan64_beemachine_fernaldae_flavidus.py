...
path = "dataset_2/training/Bombus_fernaldae_flavidus/"
...
BUFFER_SIZE = 838#200000 # number of images in training i think
BATCH_SIZE = 2#500 # This is just the standard number for batch size. Google for more info
...
generated_image = generator(train_images, training = False)
...
def generator_loss(fake_output, generated_images, images):
  fake_gen_loss = cross_entropy(tf.ones_like(fake_output), fake_output)
  autoencoder_loss = cross_entropy(images, generated_images)
  total_loss = fake_gen_loss + autoencoder_loss
  return total_loss
...
def train(dataset, epochs):  
  ...
  for epoch in range(epochs):
   ...
   # display.clear_output(wait=True)
    ...
    tf.print("Epoch:", epoch, "finished - fernaldae_flavidus")
    tf.print()
...
def generate_and_save_images(model, epoch, test_input):
  ...
  for i in range(predictions.shape[0]):
      ...
      #plt.axis('off')
      if epoch % 500 == 0:      
         plt.savefig('fernaldae_flavidus_image_at_epoch_{:04d}.png'.format(epoch))
  plt.show()
...
discriminator.save('saved_models_dcgan/discriminator_model_fernaldae_flavidus_{}.h5'.format(time_saved))
generator.save('saved_models_dcgan/generator_model_fernaldae_flavidus_{}.h5'.format(time_saved))
